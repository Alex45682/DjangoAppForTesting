...
class TestPostCompanies(BasicCompanyAPITestCase):
    ...
    def test_create_company_with_layoffs_status_should_succeed(self):
        response = self.client.post(path=self.companies_url, data={"name": "test_company_name", "status": "Layoffs"})
        self.assertEqual(response.status_code, 201)
        response_content = json.loads(response.content)
        self.assertEqual(response_content.get("status"), "Layoffs")

    def test_create_company_with_wrong_status_should_fail(self):
        response = self.client.post(path=self.companies_url, data={"name": "test_company_name", "status": "WrongStatus"})
        self.assertEqual(response.status_code, 400)
